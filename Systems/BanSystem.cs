...
public static class BanSystem
{
    ...
    {
        bool isExist = Database.user_banlist.TryGetValue(steamID, out banData);
        if (isExist)
        {
            ...
            {
                _ = Database.user_banlist.Remove(steamID);
                return false;
            }
        }
        return false;
    ...
    {
        banData = new BanData();
        User targetUserData = em.GetComponentData<User>(targetUserEntity);
        ...
        {
            bool isExist = Database.user_banlist.TryGetValue(targetUserData.PlatformId, out BanData prevBanData);
    ...
    public static bool UnbanUser(Entity userEntity)
    {
        ...
        if (isExist) _ = Database.user_banlist.Remove(userData.PlatformId);
        else return false;
        return true;
    }
