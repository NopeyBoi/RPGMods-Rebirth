...
public class ExperienceSystem
{
    ...
    public static void UpdateEXP(Entity killerEntity, Entity victimEntity)
    {
        ...
        if (isVictimNPC)
        {
            ...
            _ = Database.player_experience.TryGetValue(SteamID, out int exp);
            ...
            if (HasAllies)
            {
                ...
                foreach (KeyValuePair<Entity, float> teammate in Group)
                {
    ...
    {
        User user_component = entityManager.GetComponentData<User>(user);
        if (EXPGain > 0)
        {
            _ = Database.player_experience.TryGetValue(user_component.PlatformId, out int exp);
    ...
    {
        ServerGameManager sgm = VWorld.Server.GetExistingSystem<ServerScriptMapper>()?._ServerGameManager;
        ...
        try
        {
            foreach (Entity entity in allyBuffer)
            {
                if (entityManager.HasComponent<User>(entity))
                {
                    ...
                    float Distance = math.distance(playerPos.Position.xz, allyPos.Position.xz);
    ...
    {
        if (!Database.player_experience.TryGetValue(SteamID, out _)) Database.player_experience[SteamID] = 0;
        float level = convertXpToLevel(Database.player_experience[SteamID]);
        if (level > MaxLevel) level = MaxLevel;

        bool isLastLevel = Cache.player_level.TryGetValue(SteamID, out float level_);
        if (isLastLevel)
        {
            if (level_ < level)
            {
