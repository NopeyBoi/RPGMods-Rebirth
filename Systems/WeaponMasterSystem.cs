...
public class WeaponMasterSystem
{
    ...
    public static void UpdateMastery(Entity Killer, Entity Victim)
    {
        ...
        int MasteryValue;
        UnitStats VictimStats = em.GetComponentData<UnitStats>(Victim);
        ...
        if (em.HasComponent<PlayerCharacter>(Victim))
        {
            ...
            float BonusMastery = VictimGear.ArmorLevel + VictimGear.WeaponLevel + VictimGear.SpellLevel;
    ...
    public static void LoopMastery(Entity User, Entity Player)
    {
        ...
        _ = Cache.player_last_combat.TryGetValue(SteamID, out DateTime LastCombat);
    ...
    public static void DecayMastery(Entity userEntity)
    {
        ...
        ulong SteamID = Data.PlatformId;
        if (Database.player_decaymastery_logout.TryGetValue(SteamID, out DateTime LastDecay))
        {
    ...
    public static void BuffReceiver(Entity buffEntity, PrefabGUID GUID)
    {
        ...
        Entity Owner = em.GetComponentData<EntityOwner>(buffEntity).Owner;
        if (!em.HasComponent<PlayerCharacter>(Owner)) return;

        Entity userEntity = em.GetComponentData<PlayerCharacter>(Owner).UserEntity._Entity;
        ulong SteamID = em.GetComponentData<User>(userEntity).PlatformId;

        WeaponType WeaponType = GetWeaponType(Owner);
        bool isMastered = ConvertMastery(SteamID, WeaponType, out float PMastery, out float SMastery);
        if (isMastered)
        {
            DynamicBuffer<ModifyUnitStatBuff_DOTS> Buffer = em.GetBuffer<ModifyUnitStatBuff_DOTS>(buffEntity);
            switch (WeaponType)
            {
