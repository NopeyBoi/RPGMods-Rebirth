...
public class SquadList
{
    ...
    public static void SpawnSquad(Entity player, int group, int maxUnits)
    {
        ...
        float3 f3pos = entityManager.GetComponentData<LocalToWorld>(player).Position;
        while (total_units < maxUnits)
        {
            int unitSpawn = generate.Next(1, maxUnits - total_units);
            if (unitSpawn > 3) unitSpawn = 3;

            if (group == 0) _ = bandit_units.TryGetValue(generate.Next(1, bandit_units.Count), out unitName);
            else if (group == 1) _ = militia_units.TryGetValue(generate.Next(1, militia_units.Count), out unitName);
            else if (group == 2) _ = soldier_units.TryGetValue(generate.Next(1, soldier_units.Count), out unitName);
            else if (group == 3) _ = holy_units.TryGetValue(generate.Next(1, holy_units.Count), out unitName);
            else if (group == 4) _ = hunter_units.TryGetValue(generate.Next(1, hunter_units.Count), out unitName);
            else if (group == 5)
            {
                _ = hunter_leader.TryGetValue(generate.Next(1, hunter_leader.Count), out unitName);
                unitSpawn = 1;
            }

            bool isFound = Database.database_units.TryGetValue(unitName, out PrefabGUID unit);
            if (isFound)
            {
