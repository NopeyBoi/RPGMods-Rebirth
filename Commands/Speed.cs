...
public static class Speed
{
    public static void Initialize(Context ctx)
    {
        ...
        bool isSpeeding = Database.speeding.TryGetValue(SteamID, out _);
        if (isSpeeding) isSpeeding = false;
        else isSpeeding = true;
        _ = UpdateSpeed(ctx, isSpeeding);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId;
        bool isExist = Database.speeding.TryGetValue(SteamID, out _);
        if (isExist || !isGodMode) _ = RemoveSpeed(ctx);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId; ;
        if (Database.speeding.TryGetValue(SteamID, out _))
        {
            _ = Database.speeding.Remove(SteamID);
            return true;
        }
        return false;
    }
    ...
    public static void LoadSpeed()
    {
        ...
        {
            FileStream stream = File.Create("BepInEx/config/RPGMods/Saves/speeding.json");
            stream.Dispose();
        }
