...
public static class GodMode
{
    public static void Initialize(Context ctx)
    {
        ...
        bool isGodMode = Database.godmode.TryGetValue(SteamID, out _);
        if (isGodMode) isGodMode = false;
        else isGodMode = true;
        _ = UpdateGodMode(ctx, isGodMode);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId;
        bool isExist = Database.godmode.TryGetValue(SteamID, out _);
        if (isExist || !isGodMode) _ = RemoveGodMode(ctx);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId;
        if (Database.godmode.TryGetValue(SteamID, out _))
        {
            _ = Database.godmode.Remove(SteamID);
            return true;
        }
        return false;
    }
    ...
    public static void LoadGodMode()
    {
        ...
        {
            FileStream stream = File.Create("BepInEx/config/RPGMods/Saves/godmode.json");
            stream.Dispose();
        }
