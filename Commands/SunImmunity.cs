...
public static class SunImmunity
{
    public static void Initialize(Context ctx)
    {
        ...
        bool isSunImmune = Database.sunimmunity.TryGetValue(SteamID, out _);
        if (isSunImmune) isSunImmune = false;
        else isSunImmune = true;
        _ = UpdateImmunity(ctx, isSunImmune);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId;
        bool isExist = Database.sunimmunity.TryGetValue(SteamID, out _);
        if (isExist || !isSunImmune) _ = RemoveImmunity(ctx);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId;
        if (Database.sunimmunity.TryGetValue(SteamID, out _))
        {
            _ = Database.sunimmunity.Remove(SteamID);
            return true;
        }
        return false;
    }
    ...
    public static void LoadSunImmunity()
    {
        ...
        {
            FileStream stream = File.Create("BepInEx/config/RPGMods/Saves/sunimmunity.json");
            stream.Dispose();
        }
