...
public static class BanUser
{
    public static void Initialize(Context ctx)
    {
        ...
        if (args.Length == 1)
        {
            if (Helper.FindPlayer(args[0], false, out _, out Unity.Entities.Entity targetUserEntity_))
            {
                User targetData_ = VWorld.Server.EntityManager.GetComponentData<User>(targetUserEntity_);
                if (BanSystem.IsUserBanned(targetData_.PlatformId, out BanData banData_))
                {
        ...
        if (Helper.FindPlayer(name, false, out _, out Unity.Entities.Entity targetUserEntity))
        {
            if (BanSystem.BanUser(ctx.Event.SenderUserEntity, targetUserEntity, days, reason, out BanData banData))
            {
...
public static class UnbanUser
{
    public static void Initialize(Context ctx)
    {
        ...
        if (Helper.FindPlayer(args[0], false, out _, out Unity.Entities.Entity targetUserEntity))
        {
