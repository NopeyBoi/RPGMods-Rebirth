...
public static class SpawnNPC
{
    public static void Initialize(Context ctx)
    {
        if (ctx.Args.Length != 0)
        {
            ...
            bool isUsingGUID = int.TryParse(ctx.Args[0], out int GUID);

            if (ctx.Args.Length >= 2)
            {
                isParsable = int.TryParse(ctx.Args[1], out _);
            ...
            {
                name = ctx.Args[0];
                float3 pos = ctx.EntityManager.GetComponentData<LocalToWorld>(ctx.Event.SenderCharacterEntity).Position;
                if (isUsingGUID)
                {
            ...
            {
                name = ctx.Args[0];
                string waypoint = ctx.Args.Last().ToLower();
