...
public static class Waypoint
{
    ...
    public static void Initialize(Context ctx)
    {
        ...
        if (ctx.Args.Length > 1)
        {
            ...
            {
                string args_2nd = ctx.Args[2].ToLower();
            ...
            if (ctx.Args[0].ToLower().Equals("set"))
            {
                ...
                if (!global)
                {
                    if (Database.waypoints_owned.TryGetValue(SteamID, out int total) && !ctx.Event.User.IsAdmin)
                    {
                ...
                Unity.Mathematics.float3 location = ctx.EntityManager.GetComponentData<LocalToWorld>(ctx.Event.SenderCharacterEntity).Position;
                Float2 f2_location = new Float2(location.x, location.z);
        ...
        if (ctx.Args[0].ToLower().Equals("list"))
        {
            ...
            foreach (KeyValuePair<string, WaypointData> wp in Database.waypoints)
            {
        ...
        if (Database.waypoints.TryGetValue(waypoint + "_" + SteamID, out WaypointData WPData_))
        {
    ...
    {
        WaypointData WaypointData = new WaypointData(true_name, owner, location);
        if (isGlobal) Database.globalWaypoint[name] = WaypointData;
        else Database.waypoints[name] = WaypointData;
        if (!isGlobal && Database.waypoints_owned.TryGetValue(owner, out int total))
        {
    ...
    public static void LoadWaypoints()
    {
        ...
        {
            FileStream stream = File.Create("BepInEx/config/RPGMods/Saves/waypoints.json");
            stream.Dispose();
        }
        ...
        {
            FileStream stream = File.Create("BepInEx/config/RPGMods/Saves/global_waypoints.json");
            stream.Dispose();
        }
        ...
        {
            FileStream stream = File.Create("BepInEx/config/RPGMods/Saves/total_waypoints.json");
            stream.Dispose();
        }
