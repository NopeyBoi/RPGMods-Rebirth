...
public static class Heat
{
    ...
    public static void Initialize(Context ctx)
    {
        ...
        if (ctx.Args.Length >= 2 && isAllowed)
        {
            ...
            if (int.TryParse(ctx.Args[0], out int n)) Cache.heatlevel[SteamID] = n;
            if (int.TryParse(ctx.Args[1], out int nm)) Cache.bandit_heatlevel[SteamID] = nm;
        ...
        _ = Cache.heatlevel.TryGetValue(SteamID, out int human_heatlevel);
        ...
        _ = Cache.bandit_heatlevel.TryGetValue(SteamID, out int bandit_heatlevel);
