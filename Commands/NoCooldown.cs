...
public static class NoCooldown
{
    public static void Initialize(Context ctx)
    {
        ...
        bool isNoCD = Database.nocooldownlist.TryGetValue(SteamID, out bool _);
        if (isNoCD) isNoCD = false;
        else isNoCD = true;
        _ = UpdateCooldownList(ctx, isNoCD);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId;
        bool isExist = Database.nocooldownlist.TryGetValue(SteamID, out _);
        if (isExist || !isNoCooldown) _ = RemoveCooldown(ctx);
    ...
    {
        ulong SteamID = ctx.Event.User.PlatformId;
        if (Database.nocooldownlist.TryGetValue(SteamID, out _))
        {
            _ = Database.nocooldownlist.Remove(SteamID);
            return true;
        }
        return false;
    }
    ...
    public static void LoadNoCooldown()
    {
        ...
        {
            FileStream stream = File.Create("BepInEx/config/RPGMods/Saves/nocooldown.json");
            stream.Dispose();
        }
